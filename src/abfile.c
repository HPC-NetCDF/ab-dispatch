...
/**
 * @internal Create a AB file. Since the AB integration provides
 * read-only access to AB files, this returns error NC_ENOTNC4.
 *
 * @param path Ignored.
 * @param cmode Ignored.
 ...
 *
 * @return ::NC_ENOTNC4 Cannot create AB files.
 * @author Ed Hartnett
 */
int
AB_create(const char* path, int cmode, size_t initialsz, int basepe,
...
/**
 * @internal Given an AB type, set a pointer to netcdf type.
 *
 * See http://www.hdfgroup.org/training/HDFtraining/UsersGuide/Fundmtls.fm3.html
 * for more information re: AB types.
 *
 * @param h5 Pointer to HDF5 file info struct.
 * @param ab_typeid Type ID for ab datatype.
...
/**
 * @internal Not allowed for AB files.
 *
 * @param ncid File and group ID.
 *
...
/**
 * @internal This does nothing for AB files. Since they are
...
/**
 * @internal Does nothing, since AB files are read only. There can
